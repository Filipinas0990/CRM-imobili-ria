...
import {
    ...
    Home, Store, Building, Warehouse, TrendingUp, Key, Loader2, Check
} from "lucide-react";
            ...
            <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
                <DialogContent className="max-w-[440px] rounded-2xl bg-white dark:bg-[#161e2e] border-gray-200 dark:border-slate-700 p-6">
                    ...
                    <p className="text-sm text-gray-500 dark:text-slate-400">
                        Revise as informações antes de cadastrar o financiamento.
                    </p>
                    <div className="rounded-lg border border-gray-200 dark:border-slate-700 divide-y divide-gray-100 dark:divide-slate-700 text-sm">
                        ...
                        <div className="flex items-start gap-3 px-4 py-3">
                            ...
                            <div>
                                <p className="text-xs text-gray-400 dark:text-slate-500">Valor do imóvel</p>
                                <p className="font-medium text-gray-900 dark:text-white">
                                    {form.sob_consulta
                                        ? "Sob consulta"
                                        : form.preco
                                ...
                                {form.entrada && (
                                    <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">
                                        Entrada: R$ {Number(form.entrada).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                                        {form.parcelas ? ` · ${form.parcelas}x` : ""}
                                    </p>
                                )}
                            </div>
                        </div>
                        {form.fgts && (
                            <div className="flex items-start gap-3 px-4 py-3">
                                ...
                                <div>
                                    <p className="font-medium text-emerald-600 dark:text-emerald-400">Aceita FGTS</p>
                                </div>
                            </div>
                        )}
                    </div>
