...
// Card da IA vindo do microserviço
interface IACard {
    id: string;
    telefone: string;
    nome: string;
    ...
    return (
        <div className={`bg-white dark:bg-[#1a1d27] rounded-2xl border flex flex-col gap-3 p-4 shadow-sm hover:shadow-md transition-shadow ${temErro
            ...
            <div className="flex items-center justify-between pt-1">
                ...
                {/* Botão de reenvio manual para erros */}
                {temErro && (
                    <button
...
// ── Modal Concluir ─────────────────────────────────────────────────────────
interface ModalConcluirProps { followup: Followup; instanceName: string | null; onClose: () => void; onSaved: () => void; }
function ModalConcluir({ followup, instanceName, onClose, onSaved }: ModalConcluirProps) {
    ...
    async function handleConcluir() {
        ...
        await supabase.from("followups").update({ status: "concluido", resultado, observacao: observacao || null, data_conclusao: now }).eq("id", followup.id);
        ...
        if (criarProximo && proximaData && proximaHora) {
            ...
            await supabase.from("followups").insert({
                user_id: user?.id, lead_id: followup.lead_id, lead_nome: followup.lead_nome,
                lead_telefone: followup.lead_telefone, tipo: proximoTipo,
                data_hora: `${proximaData}T${proximaHora}:00`, status: "agendado",
                notificar_whatsapp: followup.notificar_whatsapp,
            });
        }
        setSaving(false); onSaved(); onClose();
    }
...
const FollowUps = () => {
    ...
    // ── Busca follow-ups do microserviço ──────────────────────────────────
        ...
        try {
            const res = await fetch(`${MICROSERVICO_URL}/ia-followups/reenviar`, {
                ...
                headers: {
                    ...
                    'x-api-key': MICROSERVICO_KEY,
                },
                body: JSON.stringify({ clienteId }),
            });
            const data = await res.json();
            if (data.sucesso && instanceName) {
                await fetchIAFollowups(instanceName);
            }
        } catch (err) {
    ...
    // ── Follow-ups manuais do Supabase ────────────────────────────────────
    const fetchFollowups = useCallback(async (currentUserId: string) => {
        setLoading(true);
        const { data } = await supabase
            .from("followups")
            .select("*")
            .eq("user_id", currentUserId) // RLS garantido aqui
            .order("data_hora", { ascending: true });
        setFollowups(data ?? []);
        setLoading(false);
    }, []);

    // Realtime — só follow-ups manuais do Supabase
    useEffect(() => {
        if (!userId) return;
        const channel = supabase.channel("followups-realtime")
            .on("postgres_changes", { event: "*", schema: "public", table: "followups" }, (payload) => {
                const row = payload.new as Followup;
                if (!row || row.user_id !== userId) return;
                setFollowups(prev => {
                    const exists = prev.find(f => f.id === row.id);
                    if (exists) return prev.map(f => f.id === row.id ? row : f);
                    return [...prev, row].sort((a, b) => new Date(a.data_hora).getTime() - new Date(b.data_hora).getTime());
                });
            }).subscribe();
    ...
    useEffect(() => {
        async function init() {
            ...
            const instancia = inst?.instance_name ?? `inst-${user.id.slice(0, 8)}`;
            setInstanceName(instancia);

            // Carrega estado da IA
            ...
            setIaEnabled(iaConfig?.ia_enabled ?? true);

            // Carrega dados em paralelo
            await Promise.all([
                fetchFollowups(user.id),
                fetchIAFollowups(instancia),
            ]);
        }
        init();
    }, [fetchFollowups, fetchIAFollowups]);
        ...
        try {
            ...
            if (existing) {
                await supabase.from("user_settings")
                    .update({ ia_enabled: newState, updated_at: new Date().toISOString() })
                    .eq("user_id", userId);
            } else {
                await supabase.from("user_settings").insert({ user_id: userId, ia_enabled: newState });
            }
            setIaEnabled(newState);
        } catch (err) {
            console.error("Erro ao salvar estado da IA:", err);
        } finally {
            setTogglingIA(false);
        }
    }
        ...
        <div className="h-screen flex bg-[#f5f6fa] dark:bg-[#0f1117] overflow-hidden transition-colors duration-200">
            ...
            {(showModalNovo || editingFollowup) && (
                <ModalNovo onClose={() => { setShowModalNovo(false); setEditingFollowup(null); }}
                    onSaved={() => fetchFollowups(userId)} userId={userId} editing={editingFollowup} />
            )}
            {concluindoFollowup && (
                <ModalConcluir followup={concluindoFollowup} instanceName={instanceName}
                    onClose={() => setConcluindoFollowup(null)} onSaved={() => fetchFollowups(userId)} />
            )}
            ...
            <div className="ml-16 flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-auto p-5 space-y-4">
                    ...
                    {/* Cards da IA — vêm do microserviço */}
                    {iaEnabled && (
                        <div>
                            <div className="flex items-center gap-2 mb-3">
                                <Bot className="w-4 h-4 text-blue-500" />
                                <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">
                                    Cards de Follow-up (I.A.)
                                </span>
                                {iaData && (
                    ...
                    {/* Follow-ups manuais — Supabase com RLS */}
